...
function init() {
	...
	setRenderer(queries.renderer||rendererType)
	if(!queries.renderer&&!rendererName)console.log(':(');
...
var tourLoader = function(url){
	...
	xhr.onreadystatechange = function () {
		if (xhr.readyState == 4 && xhr.status == 200){
			...
			init();
			setPanorama(panoramaID, {lon:lon, lat:lat, fov:fov}, true);
		}
	};
	xhr.send();
}
...
var load = function(){
	...
	tourLoader(tourLocation+'?'+Math.ceil(Math.random()*1e6));
}
...
var Marker = function(id){
	...
	if(!marker.type||marker.type=='panorama'){
		...
		if(marker.link.norotate)this.element.className += ' info'
